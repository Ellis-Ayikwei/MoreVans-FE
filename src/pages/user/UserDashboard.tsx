...
import { 
  faBox, faMapMarkerAlt, faUser, faStar, faSearch, faTruck, faCalendarCheck, 
  faCalculator, faBell, faExclamationCircle, faInfoCircle, faCalendarAlt, 
  faHeadset, faFileInvoice, faSun, faCloudRain, faCloud, faSnowflake, 
  faHome, faDoorOpen, faBuilding, faElevator, faStairs, faClock, faUmbrella,
  faGavel, faCommentAlt, faCheckCircle, faPaperPlane, faShieldAlt, faTimes,
  faExclamationTriangle, faCircleNotch, faAngleRight, faCreditCard
...
const UserDashboard: React.FC = () => {
  ...
  const getDisputeStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'waiting_for_info': return 'text-yellow-600 bg-yellow-100';
  ...
  const getDisputeStatusText = (status: string) => {
    switch (status) {
      ...
      case 'waiting_for_info': return 'Waiting for Info';
      default: return status.charAt(0).toUpperCase() + status.slice(1).replace('_', ' ');
    }
  };

  // Add these helper functions to your UserDashboard component
...
const getStatusText = (status: string): string => {
  switch (status) {
    case 'in_progress':
      return 'In Progress';
    default:
      // Capitalize first letter and the rest lowercase
      return status.charAt(0).toUpperCase() + status.slice(1);
  }
};

// Add this function to display weather icons
...
// Add this function near your other helper functions (after getDisputeStatusText)
...
// This would go in your component, to be used before rendering messages
...
// In your message list, add this conditionally
{isProviderTyping && (
  <div className="flex items-start mb-3">
    {activeChatProvider && (
      <img 
        src={activeChatProvider.avatar}
        alt="Provider"
        className="w-8 h-8 rounded-full mr-2 mt-2"
      />
    )}
    <div className="py-2 px-3 rounded-lg bg-white dark:bg-gray-700 shadow-sm rounded-bl-none">
  ...
  // Add this component inside your UserDashboard component
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-4 mb-4">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div className="flex items-center">
            ...
            <span className="text-gray-600 dark:text-gray-300">{propertyDetails.floors} {propertyDetails.floors > 1 ? 'Floors' : 'Floor'}</span>
          </div>
          <div className="flex items-center">
            <FontAwesomeIcon 
              icon={propertyDetails.hasElevator ? faElevator : faStairs} 
              className="text-blue-500 mr-2" 
            />
            <span className="text-gray-600 dark:text-gray-300">
              {propertyDetails.hasElevator ? 'Elevator Available' : 'No Elevator'}
            </span>
          </div>
        </div>
  ...
  // Add this component to display in the sidebar when there's an upcoming move
    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 relative overflow-hidden">
        ...
        <div className="text-sm text-gray-600 dark:text-gray-400">
          <p><FontAwesomeIcon icon={faCalendarAlt} className="mr-2" /> {new Date(booking.date).toLocaleDateString()}</p>
          <p><FontAwesomeIcon icon={faClock} className="mr-2" /> {new Date(booking.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
          {weather.precipitation > 0 && (
            <p><FontAwesomeIcon icon={faUmbrella} className="mr-2" /> {weather.precipitation}% chance of rain</p>
          )}
        </div>
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-900 min-h-screen transition-colors duration-300">
      <div className="container mx-auto px-4 py-8">
        {/* Welcome Section - Enhanced with better gradient and animations */}
        ...
        {/* Stats Cards - Upgraded with components and animations */}
<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
  ...
  <StatCard 
    ...
    value={bookings.filter(b => b.status === 'in_progress' || b.status === 'confirmed').length} 
    color="purple"
  />
  ...
  />
          ...
          <div className="w-full lg:w-1/4 space-y-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6">
              ...
              <ul className="space-y-2">
                <li>
                  <Link to="/service-request" className="flex items-center p-3 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors">
            ...
            {/* Quick Estimate Calculator - New Component */}
                  ...
                  <div key={booking.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                    ...
                    <div className="mb-4">
                      <div className="text-sm mb-1">
                        ...
                        <span className="font-medium">From:</span> {booking.pickupLocation.split(',')[0]}
                      </div>
                      <div className="text-sm">
                        <FontAwesomeIcon icon={faMapMarkerAlt} className="text-blue-500 mr-2" />
                        <span className="font-medium">To:</span> {booking.dropoffLocation.split(',')[0]}
                      </div>
                    </div>
                    ...
                    <Link 
                      to={booking.trackingUrl || '#'} 
                      className="w-full text-center block bg-blue-50 text-blue-700 py-2 rounded-lg hover:bg-blue-100 transition-colors"
                    >
                      View Live Tracking
                    </Link>
                  </div>
            ...
            {/* Upcoming Move Weather */}
            {upcomingMove && (
              <UpcomingMoveWeather booking={upcomingMove} />
            )}

            {/* Payment Alerts - Below Weather section */}
                  ...
                  <div className="flex items-start p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-900/40">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-blue-700 dark:text-blue-400 mt-0.5">
                        All your payments are up to date. View your payment history in the Billing section.
                      </p>
                    </div>
                  </div>
                )}
          ...
          <div className="w-full lg:w-3/4">
            <div className="bg-white rounded-xl shadow-md p-6">
              ...
              {filteredBookings.length === 0 ? (
                ...
                // Replace the current table with this enhanced responsive version
                  ...
                  <div className="inline-block min-w-full align-middle">
                    <div className="overflow-hidden shadow-sm ring-1 ring-black ring-opacity-5">
                      ...
                      <div className="sm:hidden mt-4 space-y-3">
                        {filteredBookings.map((booking) => (
                          <div key={booking.id} className="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-4">
                            <div className="flex justify-between items-center mb-3">
                            ...
                            <div className="text-sm text-gray-700 dark:text-gray-300 space-y-1">
                              <p><span className="font-medium">Date:</span> {new Date(booking.date).toLocaleString()}</p>
                              <p><span className="font-medium">From:</span> {booking.pickupLocation.split(',')[0]}</p>
                              <p><span className="font-medium">To:</span> {booking.dropoffLocation.split(',')[0]}</p>
                              {booking.providerName && <p><span className="font-medium">Provider:</span> {booking.providerName}</p>}
                              {booking.price && <p><span className="font-medium">Price:</span> ${booking.price.toFixed(2)}</p>}
                            </div>
                            <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700 flex justify-end space-x-3">
                              {booking.trackingUrl && (
                                <Link to={booking.trackingUrl} className="text-white bg-blue-600 px-3 py-1 rounded-md text-sm">
                                  Track
                                </Link>
                              )}
                              <Link to={`/booking/${booking.id}`} className="text-blue-600 dark:text-blue-400 px-3 py-1 text-sm">
                                Details
                              </Link>
                            </div>
                          </div>
              ...
              {activeDisputes.map(dispute => (
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className={`px-2 py-1 text-xs rounded-full ${getDisputeStatusColor(dispute.status)}`}>
                    ...
                    <div className="mt-2 pt-2 border-t border-gray-200 dark:border-gray-700">
                      <div className="flex items-center text-green-600">
                        ...
                        <span className="text-sm font-medium">
                          {dispute.resolution.outcome === 'refunded' ? 'Full refund' : 'Partially refunded'}:
                          {' '}{dispute.resolution.currency} {dispute.resolution.amount}
                        </span>
                      </div>
                    </div>
                  )}
      ...
      <div className="fixed bottom-20 right-4 z-50">
        {!showChat ? (
          ...
          <div className={`bg-white dark:bg-gray-800 rounded-lg shadow-xl overflow-hidden flex flex-col ${isChatMinimized ? 'w-72' : 'w-80 sm:w-96'}`} style={{ height: isChatMinimized ? 'auto' : '420px' }}>
            {/* Chat header */}
            <div 
                ...
                <div className="flex-1 overflow-y-auto p-4 bg-gray-100 dark:bg-gray-900">
                  {chatMessages.length > 0 ? (
                    chatMessages.map((msg) => (
                      <div 
                        key={msg.id} 
                        className={`mb-3 flex ${msg.sender === 'user' ? 'justify-end' : ''}`}
                      >
                        {msg.sender === 'provider' && (
                          <img 
                            src={activeChatProvider?.avatar || `https://ui-avatars.com/api/?name=Support`}
                            alt="Provider"
                            className="w-8 h-8 rounded-full mr-2 mt-2"
                          />
                        )}
