...
const ValidatedInput = ({ label, icon, error, ...props }: { label: string; icon?: any; error?: string; [key: string]: any }) => (
    <div className="space-y-2">
        ...
        <input {...props} className={`w-full px-4 py-2 border ${error ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500`} />
        {error && <p className="text-red-500 text-sm mt-1">{error}</p>}
    </div>
);

const SelectField = ({ label, options, value, onChange }: { label: string; options: { value: string; label: string }[]; value: string; onChange: (value: string) => void }) => (
    <div className="space-y-2">
        <label className="block text-sm font-medium text-gray-700">{label}</label>
        <select value={value} onChange={(e) => onChange(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
...
const CompanyInfo = ({ value, onChange }: { value?: { name: string; vatNumber?: string }; onChange: (value: any) => void }) => (
    <div className="space-y-4 col-span-2">
        <ValidatedInput label="Company Name" icon={faBuilding} value={value?.name || ''} onChange={(v: string) => onChange({ ...value, name: v })} />
        <ValidatedInput label="VAT Number" icon={faReceipt} value={value?.vatNumber || ''} pattern="\d{9}" onChange={(v: string) => onChange({ ...value, vatNumber: v })} />
    </div>
);

const AddressField = ({ value, onChange }: { value: UserSettings['personal']['address']; onChange: (value: UserSettings['personal']['address']) => void }) => {
    ...
    return (
        <div className="space-y-4 col-span-2">
            <ValidatedInput label="Street Address" icon={faMapMarker} value={value.street} required onChange={(v: string) => onChange({ ...value, street: v })} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <ValidatedInput label="Postal Code" value={value.postalCode} pattern="[A-Za-z0-9 ]{4,10}" icon={faUser} onChange={(v: string) => onChange({ ...value, postalCode: v })} />
            </div>
        </div>
    );
};
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="border-b border-gray-100 pb-6 mb-8">
                <SectionHeader icon={faUser} title="Personal & Business Information" description="Manage your personal details and business information" />
            </div>
                ...
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
                    ...
                    <div className="relative group">
                        <div className="h-28 w-28 rounded-full overflow-hidden border-4 border-white shadow-lg transition-transform duration-300 group-hover:scale-105">
                            <img className="h-full w-full object-cover" src={settings?.profileImage || '/default-avatar.png'} alt="Profile" />
                        </div>
                    ...
                    <div className="flex-1 text-center sm:text-left">
                        <h3 className="text-xl font-medium text-gray-800">{settings?.name || 'Complete Your Profile'}</h3>
                        <p className="text-gray-600 mb-2">{settings?.email || 'Add your contact information'}</p>
                        <div className="flex flex-wrap justify-center sm:justify-start gap-2">
                            <span className="px-3 py-1 bg-blue-100 text-blue-800 text-xs font-medium rounded-full">{settings?.isBusinessAccount ? 'Business Account' : 'Personal Account'}</span>
                            <span className="px-3 py-1 bg-green-100 text-green-800 text-xs font-medium rounded-full">{settings?.language === 'en' ? 'English' : settings?.language}</span>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div className="p-6">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <ValidatedInput
                                ...
                                onChange={(v) =>
                                    onChange('personal', 'emergencyContact', {
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 flex items-center justify-between transition-shadow duration-300 hover:shadow-md">
                ...
                <div className="flex items-center">
                    <span className="mr-3 text-sm text-gray-600">{settings?.isBusinessAccount ? 'Enabled' : 'Disabled'}</span>
                    <label className="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" checked={settings?.isBusinessAccount || false} onChange={(e) => onChange('personal', 'isBusinessAccount', e.target.checked)} className="sr-only peer" />
...
const MovingPreferencesSection = ({ settings, onChange }: { settings: UserSettings['movingPreferences']; onChange: (field: string, value: any) => void }) => (
    <div className="space-y-6">
        <SectionHeader icon={faTruckMoving} title="Moving Preferences" description="Set your default moving preferences for quick bookings" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <ToggleField label="Include Professional Packing Service" checked={settings.packingService} onChange={(v) => onChange('packingService', v)} />

            <div className="col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-2">Preferred Moving Dates</label>
                <DatePicker
                    selected={null}
            ...
            <div className="col-span-2 space-y-4">
                <label className="block text-sm font-medium text-gray-700">Preferred Time Windows</label>
                <div className="grid grid-cols-3 gap-4">
                    <ToggleField label="Morning (8am-12pm)" checked={settings.timeWindows.morning} onChange={(v) => onChange('timeWindows', { ...settings.timeWindows, morning: v })} />
                    <ToggleField label="Afternoon (12pm-5pm)" checked={settings.timeWindows.afternoon} onChange={(v) => onChange('timeWindows', { ...settings.timeWindows, afternoon: v })} />
                    <ToggleField label="Evening (5pm-9pm)" checked={settings.timeWindows.evening} onChange={(v) => onChange('timeWindows', { ...settings.timeWindows, evening: v })} />
                </div>
            </div>
...
const PaymentSection = ({ settings, onChange }) => {
    ...
    return (
        <div className="space-y-6">
            <SectionHeader icon={faCreditCard} title="Payment Methods" description="Manage your payment methods and billing information" />
            ...
            <div className="space-y-4">
                {settings?.payment?.cards.map((card) => (
                    <div key={card.id} className="p-4 bg-gray-50 rounded-lg flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-4">
                            {settings.payment.defaultPaymentId === card.id && <span className="text-sm text-green-600">Default</span>}
                            <button
                                onClick={() =>
                                    onChange(
                                        'cards',
                                        settings.payment.cards.filter((c) => c.id !== card.id)
                                    )
                                }
                ...
                {showAddCard ? (
                    <div className="p-4 bg-gray-50 rounded-lg space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <ValidatedInput label="Expiration Date (MM/YY)" value={newCard.expiry} onChange={(v) => setNewCard({ ...newCard, expiry: v })} pattern="\d{2}/\d{2}" />
                            <ValidatedInput label="CVC" value={newCard.cvc} onChange={(v) => setNewCard({ ...newCard, cvc: v })} pattern="\d{3}" />
                        </div>
                        <ValidatedInput label="Cardholder Name" value={newCard.name} onChange={(v) => setNewCard({ ...newCard, name: v })} />
                        <div className="flex space-x-4">
                            <button onClick={handleAddCard} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                                Save Card
                            </button>
                            <button onClick={() => setShowAddCard(false)} className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">
                                Cancel
                            </button>
                        </div>
                    </div>
                ) : (
                    <button onClick={() => setShowAddCard(true)} className="flex items-center text-blue-600 hover:text-blue-800">
            ...
            <div className="pt-6 border-t border-gray-200">
                <ValidatedInput label="Billing Address" icon={faMapMarker} value={settings?.payment?.billingAddress} onChange={(v) => onChange('billingAddress', v)} required />
            </div>
        </div>
    );
};
...
const SecuritySection = ({ settings, onChange }) => {
    ...
    useEffect(() => {
        if (password.new) {
            ...
            const feedbacks = ['Very weak', 'Weak', 'Medium', 'Strong', 'Very strong'];
    ...
    return (
        <div className="space-y-8">
            <SectionHeader icon={faShieldAlt} title="Security Settings" description="Manage your account security and privacy settings" />
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden transition-shadow duration-300 hover:shadow-md">
                ...
                <div className="p-6">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div className="space-y-2">
                            <div className="flex items-center">
                                <span className={`inline-flex h-4 w-4 rounded-full mr-2 ${settings?.security?.twoFactorEnabled ? 'bg-green-500' : 'bg-gray-300'}`}></span>
                                <span className="font-medium">{settings?.security?.twoFactorEnabled ? 'Enabled' : 'Disabled'}</span>
                            </div>

                            <p className="text-sm text-gray-600">
                                {settings?.security?.twoFactorEnabled ? 'Your account is protected with an additional layer of security.' : 'Add an extra layer of security to protect your account.'}
                            </p>
                            ...
                            {settings?.security?.twoFactorEnabled && (
                                <div className="mt-2 py-2 px-4 bg-blue-50 rounded-lg">
                                    <div className="flex items-center text-sm text-blue-700">
                                        ...
                                        <p>
                                            You have {settings.security.trustedDevices} trusted device{settings.security.trustedDevices !== 1 ? 's' : ''}
                                        </p>
                                    </div>
                                </div>
                            )}
                        ...
                        <button
                            ...
                            className={`px-5 py-2.5 rounded-lg transition-all duration-200 font-medium text-white ${
                                settings?.security?.twoFactorEnabled ? 'bg-red-500 hover:bg-red-600 active:bg-red-700' : 'bg-blue-600 hover:bg-blue-700 active:bg-blue-800'
                            }`}
                        >
                ...
                <div className="p-6 space-y-5">
                    <div className="relative">
                        <ValidatedInput
                            ...
                            required
                        />
                        <button type="button" className="absolute top-8 right-4 text-gray-400 hover:text-gray-600" onClick={() => setShowPassword({ ...showPassword, current: !showPassword.current })}>
                    ...
                    <div className="relative">
                        <ValidatedInput
                            ...
                            required
                        />
                        <button type="button" className="absolute top-8 right-4 text-gray-400 hover:text-gray-600" onClick={() => setShowPassword({ ...showPassword, new: !showPassword.new })}>
                    ...
                    <div className="relative">
                        ...
                        />
                        <button type="button" className="absolute top-8 right-4 text-gray-400 hover:text-gray-600" onClick={() => setShowPassword({ ...showPassword, confirm: !showPassword.confirm })}>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden transition-shadow duration-300 hover:shadow-md">
                ...
                <div className="p-6 space-y-4">
                    <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 flex items-center justify-between">
                        ...
                        <div className="text-xs">This device</div>
                    </div>
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between border-b border-gray-100 py-3">
                            <div>
                                ...
                                <p className="text-xs text-gray-500">Last active: {new Date(settings?.security?.lastLogin).toLocaleString()}</p>
                            </div>
                    ...
                    <div className="pt-3 border-t border-gray-100 flex justify-end">
                        <button className="px-4 py-2 text-sm text-white bg-red-600 hover:bg-red-700 rounded-lg transition-colors duration-200">Logout from All Devices</button>
                    </div>
                </div>
            </div>
                ...
                <div className="p-6">
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center justify-between py-3 border-b border-gray-100">
                            ...
                            <label className="relative inline-flex items-center cursor-pointer">
                                <input type="checkbox" checked={settings?.security?.loginAlerts || false} onChange={(e) => onChange('loginAlerts', e.target.checked)} className="sr-only peer" />
                                <div className="w-11 h-6 bg-gray-200 rounded-full peer peer-focus:ring-4 peer-focus:ring-blue-300 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                            </label>
                        </div>
...
const AnyVanSettings = () => {
    ...
    return (
        <div className="w-full px-4 py-8 bg-gray-50">
            ...
            <div className="relative mb-8 w-full">
                ...
                `}
                            >
                                ...
                                {activeSection === id && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-blue-600 dark:bg-blue-400 rounded-full transform translate-y-0.5" />}
                            </button>
                        ))}
                    </div>
                </div>
            ...
            {/* Feedback Messages */}
            {feedback && <div className={`mb-6 p-4 rounded-lg ${feedback.type === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{feedback.message}</div>}

            {/* Main Content - Full Width */}
            <div className="bg-white p-6 sm:p-8 rounded-xl shadow-lg w-full">
                {activeSection === 'personal' && <PersonalInfoSection settings={settings.personal} onChange={(field, value) => handleChange('personal', field, value)} errors={validationErrors} />}

                {activeSection === 'moving' && <MovingPreferencesSection settings={settings.movingPreferences} onChange={(field, value) => handleChange('movingPreferences', field, value)} />}

                {activeSection === 'payment' && <PaymentSection settings={settings.payment} onChange={(field, value) => handleChange('payment', field, value)} />}

                {activeSection === 'security' && <SecuritySection settings={settings.security} onChange={(field, value) => handleChange('security', field, value)} />}
                ...
                {unsavedChanges && (
                    <div className="mt-8 pt-6 border-t border-gray-200 flex flex-col sm:flex-row justify-between items-center gap-4">
                        ...
                        <div className="space-x-4">
                            <button onClick={() => window.location.reload()} className="px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">
                                Discard Changes
                            </button>
